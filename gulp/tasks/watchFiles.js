...
const mediaWebp = require("./mediaWebp");
const mediaAvif = require("./mediaAvif");
...
function watchFiles() {
	...
	gulp.watch(config.paths.watch.html).on("change", gulp.series(html, reload, generateSitemap));
	...
	gulp.watch(config.paths.watch.media, gulp.series(gulp.parallel(media, mediaWebp, mediaAvif, svgSprite), reload));
}
